...
class Leads(models.Model):
    ...
    partner_bank_id = fields.One2many('res.partner.bank', 'crm_id', string='Multi Documents Lines')
    rejection_note_id = fields.One2many('rejection.note.crm', 'crm_id', string='Rejection Note')
    ...
    def partner_details(self):
        if self.partner_id:
            ...
            list2 = []
            list3 = []
            for line_1 in self.partner_id.authorized_signatory_details_lines:
                if line_1:
                    list1.append((0,0, {
                    'full_name': line_1.full_name,
                    'cnic': line_1.cnic,
                    'designation' : line_1.designation,
                    'contact_no':line_1.contact_no,
                    'email_address':line_1.email_address,
                    'address':line_1.address,
                    })), 
            for line_2 in self.partner_id.key_personnel_details_lines:
                if line_2:
                    list2.append((0,0, {
                    'full_name': line_2.full_name,
                    'cnic': line_2.cnic,
                    'designation' : line_2.designation,
                    'contact_no':line_2.contact_no,
                    'email_address':line_2.email_address,
                    'address':line_2.address,
                    }))
            for line_3 in self.partner_id.bank_ids:
                if line_3:
                    list3.append((0,0, {
        ...
        if self.team_id.id == 1:
            ...
            if self.product_type == '1'  or self.product_type == '3':
                if self.stage_id.id == 6: 
                    ...
                    self['stage_id'] = 10
                    self['state'] ='pending'
                elif self.stage_id.id == 10 and self.product_type == '1':
                    self['stage_id'] = 14
                    self['state'] = 'approve'
            ...
            if self['partner_id']:
                if self.state == "approve":
                    ...
                    if self.stage_id.id == 14:
                        # line_1 = [code for code in self.key_personnel_details_lines if code]
                        # line_2 = [code2 for code2 in self.authorized_signatory_details_lines if code2]
                        # raise UserError(line_2)
                        list1 = []
                        list2 = []
                        list3 = []
                        for line_1 in self.authorized_signatory_details_lines:
                            if line_1:
                                list1.append((0,0, {
                                'full_name': line_1.full_name,
                                'cnic': line_1.cnic,
                        ...
                        for line_2 in self.key_personnel_details_lines:
                            if line_2:
                                list2.append((0,0, {
                                'full_name': line_2.full_name,
                                'cnic': line_2.cnic,
                        ...
                        for line_3 in self.partner_bank_id:
                            if line_3:
                                list3.append((0,0, {
                                    'bank_id': line_3.bank_id,
                                    'acc_number': line_3.acc_number,
        ...
        if self.team_id.id == 5:
            if self.stage_id.id == 6:
                if self.rejection_note:
                    self['state']='reject'
                    self['rejection_note_id'] = [(0,0,{
                        'time_stamp': datetime.now(),
                        'rejected_by': self.user_id.id,
                        'rejection_note':self.rejection_note
                    })] 
                    raise UserError(self['rejection_note_id'].id)
